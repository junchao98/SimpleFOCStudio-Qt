...
ConfigureConnectionDialog::ConfigureConnectionDialog(QWidget* parent) : QDialog(parent)
{
...
double ConfigureConnectionDialog::stopBits() const
{
    return m_stopBitsCombo->currentText().toDouble();
}
...
{
    if(p == "E")
        m_parityCombo->setCurrentText("Even");
    else if(p == "O")
        m_parityCombo->setCurrentText("Odd");
    else
        m_parityCombo->setCurrentText("None");
}
void ConfigureConnectionDialog::setByteSize(int s)
{
    m_byteSizeCombo->setCurrentText(QString::number(s));
}
void ConfigureConnectionDialog::setStopBits(double s)
{
    m_stopBitsCombo->setCurrentText(QString::number(s));
}
