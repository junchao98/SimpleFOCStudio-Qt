    ...
    , m_device(SimpleFOCDevice::instance())
{
    ...
    connect(connectBtn, &QPushButton::clicked, [this, portCombo, baudEdit, parityCombo, byteCombo, stopCombo]() {
        QVariantMap config;
        ...
        config["serialParity"] = parityCombo->currentText().startsWith('E') ? "E" :
                                 parityCombo->currentText().startsWith('O') ? "O" : "N";
...
QIcon CommandLineTool::getTabIcon() const
{
