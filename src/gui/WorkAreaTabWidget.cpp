...
WorkAreaTabWidget::WorkAreaTabWidget(QWidget* parent)
    : QTabWidget(parent), m_device(SimpleFOCDevice::instance())
{
...
void WorkAreaTabWidget::openDevice()
{
    if(m_configDeviceTool)
        return;
    QString fileName =
        QFileDialog::getOpenFileName(this, "Open Device Configuration", "", "JSON Files (*.json)");
    if(fileName.isEmpty())
        return;
    ...
    {
        QMessageBox::warning(this, "SimpleFOC ConfigTool", "Error while opening selected file");
    }
}

void WorkAreaTabWidget::saveDevice()
{
    if(m_activeToolsList.isEmpty())
        return;
    ...
    if(m_device->openedFile.isEmpty())
    {
        QString fileName = QFileDialog::getSaveFileName(
            this, "Save device configuration", "", "JSON configuration file (*.json)");
        if(fileName.isEmpty())
            return;
