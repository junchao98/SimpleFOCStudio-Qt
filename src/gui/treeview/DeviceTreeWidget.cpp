    ...
    , m_device(SimpleFOCDevice::instance())
{
    ...
    connect(m_device, &SimpleFOCDevice::configurationUpdated,
            this, &DeviceTreeWidget::onConfigurationUpdated);
    connect(m_device, &SimpleFOCDevice::connectionStateChanged,
            this, &DeviceTreeWidget::onConnectionStateChanged);
        ...
        if (item && item->data(0, Qt::UserRole).toString() == "custom_command") {
            ...
            if (chosen == addAct) {
                ...
                if (idx >= 0 && idx < m_device->customCommands.size() && m_device->getIsConnected()) {
                    m_device->sendCommand(m_device->devCommandID + m_device->customCommands[idx].cmd);
                }
            }
        }
    });
...
void DeviceTreeWidget::addMotionConfig(QTreeWidgetItem *parent)
{
    ...
    createComboItem(item, "Motion Control Type",
                    {"Torque", "Velocity", "Angle", "Velocity openloop", "Angle openloop"},
                    m_device->controlType, "controlType");
    createComboItem(item, "Torque Control Type",
                    {"Voltage", "DC Current", "FOC Current"},
                    m_device->torqueType, "torqueType");
    createEditableItem(item, "Motion Downsample",
                       QString::number(m_device->motionDownsample), "motionDownsample");
}

void DeviceTreeWidget::addPIDGroup(QTreeWidgetItem *parent, const QString &name,
...
void DeviceTreeWidget::addLimits(QTreeWidgetItem *parent)
{
    ...
    createEditableItem(item, "Velocity Limit", QString::number(m_device->velocityLimit), "velocityLimit");
    createEditableItem(item, "Voltage Limit", QString::number(m_device->voltageLimit), "voltageLimit");
    createEditableItem(item, "Current Limit", QString::number(m_device->currentLimit), "currentLimit");
}
...
void DeviceTreeWidget::addSensorConfig(QTreeWidgetItem *parent)
{
    ...
    createEditableItem(item, "Electrical Zero", QString::number(m_device->sensorElectricalZero), "sensorElectricalZero");
    createEditableItem(item, "Zero Offset", QString::number(m_device->sensorZeroOffset), "sensorZeroOffset");
}
...
void DeviceTreeWidget::addGeneralSettings(QTreeWidgetItem *parent)
{
    ...
    createEditableItem(item, "Phase Resistance", QString::number(m_device->phaseResistance), "phaseResistance");
}
...
void DeviceTreeWidget::addCustomCommands(QTreeWidgetItem *parent)
{
    ...
    for (int i = 0; i < m_device->customCommands.size(); ++i) {
        QTreeWidgetItem *cmdItem = new QTreeWidgetItem(item, {m_device->customCommands[i].cmdName, m_device->customCommands[i].cmd});
...
QTreeWidgetItem *DeviceTreeWidget::createEditableItem(QTreeWidgetItem *parent, const QString &label,
...
QTreeWidgetItem *DeviceTreeWidget::createComboItem(QTreeWidgetItem *parent, const QString &label,
                                                    ...
                                                    const QString &propertyKey)
{
    ...
    connect(combo, QOverload<int>::of(&QComboBox::currentIndexChanged), [this, propertyKey](int idx) {
        if (m_updating || !m_device->getIsConnected()) return;
        if (propertyKey == "controlType") m_device->sendControlType(idx);
        else if (propertyKey == "torqueType") m_device->sendTorqueType(idx);
    });
...
void DeviceTreeWidget::onConnectionStateChanged(bool connected)
{
    Q_UNUSED(connected);
}
